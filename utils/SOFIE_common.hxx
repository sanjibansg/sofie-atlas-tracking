...
public:
   ...
   // Constructor accepting std::span (C++20)
   InitializedTensor(ETensorType type, std::span<const std::size_t> shape, std::shared_ptr<void> data, bool typeConstant = false)
      : fConstant(typeConstant), fType{type}, fShape{shape.begin(), shape.end()}, fData{data}
   {
   }
   
   // Constructor accepting std::vector for convenience
   InitializedTensor(ETensorType type, const std::vector<std::size_t>& shape, std::shared_ptr<void> data, bool typeConstant = false)
      : fConstant(typeConstant), fType{type}, fShape{shape}, fData{data}
   {
   }
...
T* CreateBroadcastTensor(const T* data, const std::vector<size_t>& shape, const std::vector<size_t>& targetShape, size_t targetLength) {
   ...
   std::span<T> bData(broadcastedData, targetLength);
...
void UnidirectionalBroadcast(const T* data, const std::vector<size_t>& shape, const std::vector<size_t>& targetShape, std::span<T> broadcastedData) {
   ...
   if (shape.size() < targetShape.size()) {
      ...
      BroadcastTensor<T, std::span<const T>>(inData, shape, targetShape, broadcastedData);
   }
}
